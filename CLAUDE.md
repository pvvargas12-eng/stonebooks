...
## Sprint FIELD-PUSH (2026-07-18) — BUILT, needs activation
...
- **Owner nav follow-up (same day, three Paul directives):** the owner build regained the **Work hub** (JOBS tab), kept one-tap **ORDERS** (FindScreen `mode 'orders'` re-mounted; FIND → Search duplicates it deliberately — Paul's call), and **dropped the raised (+) center button** ("I said remove (+) — in the tasks I should be able to add new task"). Final owner bar: **Today / Tasks / Jobs / Orders / Find — five plain slots, no center CTA**; new tasks come from the Tasks screen's "+ New task" and Today's My-tasks card (both pre-existing). Crew bar unchanged: Today / Tasks / [raised camera] / Jobs / Find.
