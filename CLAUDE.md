...
- **Owner nav follow-up (same day, Paul: "where's my home button with options?"):** the owner build regained the **Work hub** — both builds now share the five tab slots (Today / Tasks / center / **Jobs** / Find). The owner-only ORDERS tab was removed as a duplicate: FIND → Search is the same OrdersScreen with money for owners. FindScreen `mode 'orders'` kept but unmounted.
