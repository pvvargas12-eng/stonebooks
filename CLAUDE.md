...
- **`getBringUpReady()`** (stonebooksData, by countCutReady): queued new-stone pieces (job_components track new_stone, on_floor=false, job-linked) where **design approved** (proof_approved milestone done OR current proof.approved_at) + **stone here** (stone_received OR stone_in_stock done; NO stone keys at all → pass, the cut-list rule) + **contracted** (the isRealWork gate via new shared `_filterRealWorkJobIds` — chunked .in(150); drafts/leads excluded per standing doctrine). Returns `{count, jobIds}`; **count = PIECES** (die + base separately) so every badge matches what the board lists.
- **ProductionFloor board**: red pulsing `.pf-alert` section on the New Stone tab (top 30 rows, one-click "⤒ Bring up" → first column via the existing pullUp) + red count pill on the New Stone track chip. **Hand-picked rule intact — the alert nags, Paul pulls.**
