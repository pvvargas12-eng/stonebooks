...
- **Owner nav follow-up (same day, Paul: "where's my home button with options?" then "Build it"):** the owner build regained the **Work hub** AND kept one-tap Orders — owner bar is now **six slots** (Today / Tasks / center / **Jobs** / **Orders** / Find); crew keeps the approved five. FindScreen `mode 'orders'` re-mounted as the owner ORDERS tab; FIND → Search remains the same orders list, so the duplication is deliberate (Paul's call).
