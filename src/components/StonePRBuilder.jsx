// =============================================================================
// StonePRBuilder — guided flow to build a Stone Purchase Request.
// =============================================================================
// Pick/create a supplier → add line items (manually OR pulled from open-order needs
// with no yard match) → PR fields → Save (creates a bulk_orders draft + items).
// =============================================================================

import { useState, useEffect, useCallback } from 'react'
import { rowToOrder } from '../SalesMode'
import {
  listSuppliers, createSupplier, createStonePR, getActiveStoneOrders, getInventoryStock,
  getCurrentStaffName, SUPPLIER_KINDS,
...
export default function StonePRBuilder({ onClose, onSaved, prefillLines = null }) {
  ...
  useEffect(() => {
    ...
    // Pull "needs" = open-order stone needs with no available yard match.
    Promise.all([getActiveStoneOrders(), getInventoryStock()]).then(([ordRes, stockRes]) => {
      if (!alive) return
      const stock = stockRes.rows || []
      const orders = (ordRes.rows || []).map(r => { const o = rowToOrder(r, null, null); o.family = familyOf(r); return o })
  ...
  const saveNewSupplier = async () => {
    ...
    setShowNewSup(false)
    setNewSup({ name: '', contact_name: '', phone: '', email: '', terms: '', lead_time_days: '', kinds: ['stone'], notes: '' })
  }

  const addDraft = () => {
    if (!draft.family_name.trim() && !draft.color.trim() && !draft.size.trim()) return
    ...
    <div className="prb-overlay" onClick={onClose}>
      ...
      <div className="prb-modal" onClick={e => e.stopPropagation()}>
        <div className="prb-head">
          <div><div className="prb-eyebrow">Procurement · Stone</div><h2 className="prb-title">Build a Purchase Request</h2></div>
        ...
        <div className="prb-body">
          ...
          <section className="prb-sec">
            <div className="prb-sec-title">1 · Supplier</div>
            ...
            {showNewSup && (
              <div className="prb-newsup">
                <div className="prb-grid">
                  <Field label="Name *"><input className="sb-input" value={newSup.name} onChange={e => setNewSup(s => ({ ...s, name: e.target.value }))} placeholder="Peerless" /></Field>
          ...
          <section className="prb-sec">
            ...
            {needs.length > 0 && (
              <div className="prb-needs">
                <button type="button" className="prb-needs-toggle" onClick={() => setShowNeeds(v => !v)}>
                  {showNeeds ? '▾' : '▸'} Pull from open-order needs ({needs.length} not in the yard)
                </button>
                {showNeeds && (
            ...
            {lines.length > 0 && (
              <table className="prb-lines">
                <thead><tr><th>Family</th><th>Color</th><th>Size</th><th>Top</th><th>Sides</th><th>Qty</th><th>Notes</th><th /></tr></thead>
                <tbody>
                  {lines.map((l, i) => (
                    <tr key={i}>
                      <td>{l.family_name || '—'}</td><td>{l.color || '—'}</td><td className="prb-mono">{l.size || '—'}</td>
                      <td>{l.top || '—'}</td><td>{l.sides || '—'}</td><td className="prb-num">{l.quantity}</td><td>{l.notes || ''}</td>
