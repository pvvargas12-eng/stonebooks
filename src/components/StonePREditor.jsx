...
import { resolvePRLineSpecs } from '../lib/prSpec'
...
export default function StonePREditor({ bulkOrderId, onClose, onSaved }) {
  ...
  const addRow = () => setRows(rs => [...rs, { id: tmpId(), family_name: '', quantity: 1, spec_text: '', live: '—', isNew: true }])
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const r of rows) {
        if (r.isNew) {
          if (!r.family_name.trim() && !r.spec_text.trim()) continue   // skip blank new rows
          const a = await addBulkOrderItem(bulkOrderId, { family_name: r.family_name, quantity: r.quantity, spec_text: r.spec_text })
          ...
          {loading ? <div className="sb-empty">Loading lines…</div>
            ...
            : (
              <>
                ...
                <button type="button" className="pre-add" onClick={addRow}>+ Add line</button>
...
const PRE_CSS = `
  ...
  .pre-add { margin-top: 12px; background: none; border: 1px dashed var(--sb-border, #d8d2c4); border-radius: 8px; font: inherit; font-size: 13px; font-weight: 600; color: #6b5d3a; padding: 8px 14px; cursor: pointer; }
