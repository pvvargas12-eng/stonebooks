...
import { listNotifications, markRead, markAllRead } from '../lib/notificationsFeed'
import { getPushState } from './fieldPush'
...
export default function NotificationsScreen({ who, undo, onOpenJob, onOpenTask, onBack, onAskPermission }) {
  ...
  const [pushState, setPushState] = useState('on')        // banner hidden until checked
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setRows(list || [])
        setToday(todayISO())
        getPushState().then(r => { if (!cancelled) setPushState(r.state) }).catch(() => {})
      } catch (e) {
  ...
  const openItem = useCallback((n) => {
    ...
    // Rows carry the same URL the push payload does: /field?task= | ?order=
    const dl = String(n.deep_link || '')
    let m
    if ((m = dl.match(/[?&]task=([^&]+)/))) {
      if (onOpenTask) onOpenTask(decodeURIComponent(m[1]))
    } else if ((m = dl.match(/[?&]order=([^&]+)/))) {
      if (onOpenJob) onOpenJob({ orderId: decodeURIComponent(m[1]), jobId: null }, 'today')
    }
  ...
  const showBanner = pushState === 'off' || pushState === 'denied' || pushState === 'needs-install'
            ...
            <div style={{ background: '#FDF6E8', border: '1px solid #EAD9AE', borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              <div style={{ flex: 1, fontSize: 13.5, fontWeight: 600, color: '#6B5A23', lineHeight: 1.45 }}>
                {pushState === 'needs-install'
                  ...
                  : pushState === 'denied'
                    ? 'Notifications are blocked for this app — allow them in the phone’s Settings.'
                    : 'Notifications are off on this phone — nothing reaches your lock screen.'}
              </div>
              {onAskPermission && pushState === 'off' && (
