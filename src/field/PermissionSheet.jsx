...
import { subscribeThisPhone, pushCapability } from './fieldPush'

const SNOOZE_KEY = 'sb_field_push_snooze'
const SNOOZE_DAYS = 7

// Should the shell auto-offer this sheet right now? Only when the ask could
// still go somewhere: the phone can push (fieldPush capability — iOS requires
// the installed PWA), permission untouched, and not dismissed within the last
// 7 days. Called from effects/handlers only.
...
export default function PermissionSheet({ who, onClose, onGranted }) {
  ...
  const turnOn = () => {
    ...
    Promise.resolve(req).then(async (perm) => {
      if (perm === 'granted') {
        const res = await subscribeThisPhone(who)
        setBusy(false)
